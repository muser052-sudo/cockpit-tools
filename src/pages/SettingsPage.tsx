...
export function SettingsPage() {
  ...
  const [proxyPort, setProxyPort] = useState('19530');
  const [proxyAutoStart, setProxyAutoStart] = useState(false);
  const [proxyApiKey, setProxyApiKey] = useState('');
  const [proxyRunning, setProxyRunning] = useState(false);
  const [proxyActualPort, setProxyActualPort] = useState<number | null>(null);
  const [proxyProviders, setProxyProviders] = useState<Record<string, ProviderProxyConfig>>({
    antigravity: { enabled: false, strategy: 'round_robin', account_ids: [] },
    codex: { enabled: false, strategy: 'round_robin', account_ids: [] },
  });
  ...
  const loadNetworkConfig = async () => {
    ...
    // 加载代理配置
    try {
      const proxyConfig = await invoke<ApiProxyConfig>('get_api_proxy_config');
      setProxyEnabled(proxyConfig.enabled);
      setProxyPort(String(proxyConfig.port));
      setProxyAutoStart(proxyConfig.auto_start);
      setProxyApiKey(proxyConfig.api_key || '');
      setProxyProviders(proxyConfig.providers || {
        antigravity: { enabled: false, strategy: 'round_robin', account_ids: [] },
        codex: { enabled: false, strategy: 'round_robin', account_ids: [] },
      });
    } catch (err) {
    ...
    try {
      ...
      setProxyActualPort(status.actual_port);
    } catch (err) {
  ...
  const handleSaveProxyConfig = async () => {
    setProxySaving(true);
    try {
      const portNum = parseInt(proxyPort, 10) || 19530;
      const config: ApiProxyConfig = {
        enabled: proxyEnabled,
        port: portNum,
        allow_lan_access: false,
        api_key: proxyApiKey,
        auto_start: proxyAutoStart,
        request_timeout: 120,
        providers: proxyProviders,
      };
      await invoke('save_api_proxy_config', { config });
      alert('代理配置已保存');
    } catch (err) {
      alert(`保存代理配置失败: ${err}`);
    } finally {
  ...
  // 更新 Provider 配置
  const updateProviderConfig = (provider: string, updates: Partial<ProviderProxyConfig>) => {
        ...
        <div className="settings-content">
          ...
          {activeTab === 'network' && (
            <>
              ...
              {/* API 反向代理配置 */}
              <div className="group-title" style={{ marginTop: '24px' }}>API 反向代理</div>
              <div className="settings-group">
                <div className="settings-row">
                  <div className="row-label">
                    <div className="row-title">代理服务</div>
                    <div className="row-desc">启用 HTTP 反向代理，将请求转发到上游 AI 服务并自动轮询账号</div>
                  </div>
                  <div className="row-control" style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                    <button
                      className={`btn ${proxyRunning ? 'btn-secondary' : 'btn-primary'}`}
                      onClick={handleToggleProxy}
                      disabled={proxyStarting}
                      style={{ fontSize: '13px', padding: '6px 14px' }}
                    >
                      {proxyStarting ? '...' : proxyRunning ? '⏹ 停止' : '▶ 启动'}
                    </button>
                    <span style={{
                      display: 'inline-block',
                      width: '8px', height: '8px',
                      borderRadius: '50%',
                      background: proxyRunning ? '#22c55e' : '#94a3b8',
                    }}></span>
                  </div>
                </div>
                ...
                <div className="settings-row" style={{ animation: 'fadeUp 0.3s ease both' }}>
                  <div className="row-label">
                    <div className="row-title">监听端口</div>
                    <div className="row-desc">默认为 19530，修改后需重新启动代理</div>
                  </div>
                  <div className="row-control">
                    <input
                      type="number"
                ...
                {proxyRunning && proxyActualPort && (
                  <div className="settings-row" style={{ animation: 'fadeUp 0.3s ease both' }}>
                    <div className="row-label">
                      <div className="row-title">当前运行端口</div>
                      <div className="row-desc">代理服务正在运行中</div>
                    </div>
                    <div className="row-control">
                      <span style={{
                        fontFamily: 'var(--font-mono)',
                        ...
                        http://127.0.0.1:{proxyActualPort}
                      </span>
                    </div>
                  </div>
                )}
                ...
                <div className="settings-row">
                  <div className="row-label">
                    <div className="row-title">自动启动</div>
                    <div className="row-desc">应用启动时自动开启代理服务</div>
                  </div>
                  <div className="row-control">
                    <label className="switch">
                ...
                <div className="settings-row">
                  <div className="row-label">
                    ...
                    <div className="row-desc">为空则不验证请求。设置后客户端需携带 Authorization: Bearer &lt;key&gt;</div>
                  </div>
                  <div className="row-control row-control--grow">
                    <input
                      type="text"
              ...
              {/* Provider 配置 */}
              <div className="group-title" style={{ marginTop: '16px' }}>Provider 配置</div>
              <div className="settings-group">
                {Object.entries(proxyProviders).map(([key, cfg]) => (
                  <div key={key}>
                    <div className="settings-row">
                      <div className="row-label">
                        <div className="row-title" style={{ textTransform: 'capitalize' }}>{key}</div>
                        <div className="row-desc">启用 {key} 的 API 反向代理</div>
                      </div>
                      <div className="row-control">
                        <label className="switch">
                          <input
                    ...
                    {cfg.enabled && (
                      <div className="settings-row" style={{ animation: 'fadeUp 0.3s ease both', paddingLeft: '16px' }}>
                        <div className="row-label">
                          <div className="row-title">负载均衡策略</div>
                          <div className="row-desc">多账号请求分配方式</div>
                        </div>
                        <div className="row-control">
                          <select
                            className="settings-select"
                            value={cfg.strategy}
                            onChange={(e) => updateProviderConfig(key, { strategy: e.target.value })}
                          >
                            <option value="round_robin">轮询 (Round-Robin)</option>
                            <option value="random">随机 (Random)</option>
                            <option value="single">单账号 (Fixed)</option>
                          </select>
                        </div>
                      </div>
                    )}
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '12px' }}>
                ...
                >
                  <Save size={16} /> {proxySaving ? '保存中...' : '保存代理配置'}
                </button>
              </div>
            </>
          )}
